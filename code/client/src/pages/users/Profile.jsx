        ...
        {user ? (
          <section className="w-full h-full flex flex-col items-center justify-center gap-5">
            ...
            <div className="w-full h-full flex flex-col items-center justify-center gap-5 sm:flex-row">
              {/* The profile image */}
              <div className="bg-white relative p-3 flex justify-center items-center w-full h-52 rounded-3xl shadow-lg hover:shadow-xl transition-all duration-300 sm:h-66">
                <img
                  src={user?.profilePic}
              ...
              {/* The details section */}
              <div className="bg-white p-5 flex flex-col justify-center items-center gap-5 w-full h-auto rounded-3xl shadow-lg hover:shadow-xl transition-all duration-300 sm:h-66">
                {/* The details */}
                <div className="flex flex-col items-start justify-center text-center gap-1">
                  <h2 className="text-2xl font-semibold text-gray-900 sm:text-3xl">
                    {`${user?.fullName?.firstName} ${user?.fullName?.lastName}`}
                  </h2>
                  <h5 className="text-sm text-gray-600 sm:text-md">
                    {user?.email}
                  </h5>
                  <h5 className="text-xs text-gray-500 italic sm:text-sm">
                    @{user?.username}
                  </h5>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  <Link to={"/users/me/details"} className="cursor-pointer">
                    <Button
                      ...
                      styles="rounded-2xl text-sm p-5 font-semibold tracking-wide shadow-md hover:shadow-lg transition-all duration-300 sm:p-6 sm:text-lg"
